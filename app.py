...
from utils import generate_qr, load_qr_code
...
# -------------------------------------
    ...
    for order in orders:
        st.write(
            f"Order #{order['id']} | Table: {order['table_id']} | Items: {order['items']} | "
            f"Instructions: {order['instructions']} | Status: {order['status']} | Time: {order['timestamp']}"
        )
...
# -------------------------------------
    ...
    if login_admin():
        ...
        st.subheader("📎 Generate Table QR Code")
